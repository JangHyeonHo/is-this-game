...
**설계 항목은 [확정] / [방향] / [검토중] / [제안] 표기를 반드시 구분하세요.**
[확정]만 구현 착수 가능합니다. [방향]은 방향만 합의된 것이라 세부가 없고,
[제안]은 에이전트가 낸 안일 뿐 승인된 적이 없습니다.

제안을 사용자의 결정처럼 인용하면, 사용자는 기억하지도 못하는 결정을 근거로
작업이 진행됩니다. 실제로 그 사고가 났습니다. 눈금을 건너뛰는 것도 같은 사고입니다 —
방향만 합의된 것을 [확정]으로 올리지 마세요. 표기가 없으면 확정으로 취급하지 마세요.
