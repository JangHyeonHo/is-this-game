...
값 자체는 여기에 적지 않는다. 수치는 코드가 정본이고, 왜 그 숫자인지는
[08-balance-log.md](08-balance-log.md)가 답한다.

## 0. ⚠️ 표시의 뜻

밸런스 수치는 대부분 아직 검증되지 않았다. 검증 안 된 값에는 코드에 이 표시가 붙어 있다.

```csharp
/// <summary>
/// ⚠️ 임시값 — 배치 시뮬레이션으로 검증하고 근거를 docs/08-balance-log.md에 남기세요.
/// </summary>
```

이 표시가 있는 값은 시뮬레이션으로 검증한 뒤에 고친다. 남아 있는 임시값 목록은
`08-balance-log.md` #41 · #51에 있다.
...
| 마나가 실제로 제약이 되는가 | 8달 파견에서 제약이 된 달이 0이었다 (`08` #48) |
| 난이도 6 이상 전투 | 견습 파티가 진다. 절대 수치 문제다 (`08` #43) |

자세한 것은 [05-gaps.md](05-gaps.md)의 "제약이 기능을 막고 있는 곳"에 있다.
