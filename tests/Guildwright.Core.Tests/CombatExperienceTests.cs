...
public class CombatExperienceTests(ITestOutputHelper output)
{
    ...
    private static Adventurer Veteran(WeaponStyle style)
    {
        ...
        var a = new Adventurer(
            "V", "실험체", StatBlock.Uniform(20), 50, Profile(), 20,
    ...
    public void 경험은_총_성장량이_아니라_방향을_바꾼다()
    {
        ...
        double tankSum = StatBlock.AllKinds.Sum(tank.WeightOf);
        double mageSum = StatBlock.AllKinds.Sum(mage.WeightOf);

        output.WriteLine($"탱커 가중치 합 {tankSum:F2} / 마법사 가중치 합 {mageSum:F2} (균등은 7.00)");

        Assert.InRange(tankSum, 5.5, 8.5);
        Assert.InRange(mageSum, 5.5, 8.5);
    }
    ...
    public void 앞에서_맞은_캐릭터는_체력과_방어가_자란다()
    {
        ...
        Assert.True(tank.WeightOf(StatKind.Vitality) > 1.0);
        Assert.True(tank.WeightOf(StatKind.Defense) > 1.0);
        Assert.True(tank.WeightOf(StatKind.MagicAttack) < 1.0);
    }
    ...
    public void 마법을_쓴_캐릭터는_마력과_마공이_자란다()
    {
        ...
        Assert.True(mage.WeightOf(StatKind.MagicAttack) > 1.0);
        Assert.True(mage.WeightOf(StatKind.Mana) > 1.0);
        Assert.True(mage.WeightOf(StatKind.Attack) < 1.0);
    }
    ...
    public void 쓰지_않은_능력치도_최소한은_자란다()
    {
        ...
        Assert.True(mage.WeightOf(StatKind.Attack) > 0.2);
    }
    ...
    public void 역할이_다르면_실전_5년_후_전혀_다른_캐릭터가_된다()
    {
        ...
        StatBlock After(WeaponStyle style)
        {
            var a = Veteran(style);
        ...
        Assert.True(tank.Defense > mage.Defense,
            "앞에서 맞은 쪽의 방어가 더 높아야 합니다.");
        Assert.True(mage.MagicAttack > tank.MagicAttack,
            "마법을 쓴 쪽의 마공이 더 높아야 합니다.");
        Assert.True(mage.Mana > tank.Mana);
    }
    ...
    public void 실전_기록을_직접_넘기면_그대로_반영된다()
    {
        ...
        Assert.True(gain.MagicAttack > gain.Attack,
    ...
    public void 전투_기록에서_만든_경험이_역할을_반영한다()
    {
        ...
        double Defensive(CombatExperience e) =>
            e.WeightOf(StatKind.Vitality) + e.WeightOf(StatKind.Defense) + e.WeightOf(StatKind.MagicDefense);
        ...
        Assert.True(mageExp.WeightOf(StatKind.MagicAttack) > tankExp.WeightOf(StatKind.MagicAttack));
    }
}
